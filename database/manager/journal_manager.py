...
class JournalManager(BaseManager):
    ...
    def _generate_next_entry_number(self):
        ...
        try:
            ...
            # الحصول على أكبر رقم قيد مع التحقق من صحة التنسيق
            ...
            if not max_entry_number:
                return "1"
            
            # إذا كان الرقم مجرد أرقام بدون حروف
            if max_entry_number.isdigit():
                next_num = int(max_entry_number) + 1
                return str(next_num)
            
            # إذا كان الرقم يحتوي على حروف وأرقام
            num_part_str = ''.join(filter(str.isdigit, max_entry_number))
            alpha_part = ''.join(filter(str.isalpha, max_entry_number))
            
            if num_part_str:
                num_part = int(num_part_str)
                next_num = num_part + 1
                
                # الحفاظ على تنسيق الأصفار إذا كان موجوداً
                if num_part_str.startswith('0'):
                    padding = len(num_part_str)
                    return f"{alpha_part}{next_num:0{padding}d}"
                else:
                    return f"{alpha_part}{next_num}"
            else:
                # إذا كان الرقم يحتوي فقط على حروف
                return f"{max_entry_number}1"
    ...
    def get_next_entry_number(self, conn=None):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT MAX(CAST(entry_number AS INTEGER)) FROM journal_entries WHERE entry_number GLOB '[0-9]*'")
            max_numeric_value = cursor.fetchone()[0]

            if max_numeric_value is None:
                return "1"
            
            # البدء من الرقم التالي والتحقق من التكرار
            next_num = max_numeric_value + 1
            attempts = 0
            
            while attempts < 100:  # حد أقصى للمحاولات لتجنب التكرار اللانهائي
                # التحقق مما إذا كان الرقم موجوداً بالفعل
                cursor.execute("SELECT COUNT(*) FROM journal_entries WHERE entry_number = ?", (str(next_num),))
            ...
            # إذا فشلنا في إيجاد رقم فريد بعد 100 محاولة
            QMessageBox.critical(None, "Error", "Failed to generate unique entry number after 100 attempts")
            return None
    ...
    def add_journal_entry(self, entry_date, description, transaction_type_id, total_debit, 
                     total_credit, lines, status, system_date, financial_year_id, created_by=None):
        conn = None
        try:
            conn = self.get_connection()
            if conn is None: 
            ...
            insert_query = """
                ...
                (entry_number, entry_date, system_date, financial_year_id, description, 
                transaction_type_id, total_debit, total_credit, status, created_by, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            """
            cursor.execute(insert_query, (
                entry_number, entry_date, system_date, financial_year_id, description,
                transaction_type_id, total_debit, total_credit, status, created_by
            ))
            ...
            for line in lines:
                cursor.execute("""
                    ...
                    (journal_entry_id, account_id, debit, credit, document_type_id, 
                    document_number, tax_type_id, cost_center_id, notes)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
    ...
    def update_journal_entry(self, entry_id, entry_date, description, transaction_type_id, 
                        total_debit, total_credit, lines, system_date, financial_year_id, updated_by=None):
        conn = None
        try:
            conn = self.get_connection()
            if conn is None: 
            ...
            update_query = """
                ...
                SET entry_date = ?, description = ?, transaction_type_id = ?,
                    total_debit = ?, total_credit = ?, system_date = ?, financial_year_id = ?,
    ...
    def get_journal_entry_by_id(self, entry_id):
        ...
        try:
            ...
            QMessageBox.critical(None, "Database Error", f"Error fetching journal entry: {e}")
            return None
        finally:
            if conn:
                conn.close()
    ...
    def get_all_journal_entries(self):
        ...
        try:
            ...
            cursor.execute("""
                ...
                LEFT JOIN Users UU ON JE.updated_by = UU.id
    ...
    def search_journal_entries(self, search_term):
        ...
        try:
            ...
            query = """
                ...
                LEFT JOIN Users U ON JE.created_by = U.id
                LEFT JOIN Users UU ON JE.updated_by = UU.id
                WHERE JE.entry_number LIKE ? COLLATE NOCASE 
    ...
    def get_all_financial_years(self):
        ...
        try:
            ...
            columns = [description[0] for description in cursor.description]
            return [dict(zip(columns, year)) for year in years]
        except sqlite3.Error as e:

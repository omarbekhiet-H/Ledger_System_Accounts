...
# ================================================================
# استيراد نافذة تسجيل الدخول الجديدة والمدير الجديد
...
from ui.financial.NEW_TRANSACTION import NewJournalEntryManagementWindow
...
class ApplicationController:
    def __init__(self):
        ...
        self.user_manager = NEWUserManager()  # إنشاء مدير المستخدمين
        self.login_window = LoginWindow(self.user_manager)  # تمرير المدير لنافذة التسجيل
        self.login_window.login_success.connect(self.show_main_window)  # ربط إشارة النجاح
    ...
    def show_main_window(self, user_data):
        """عرض النافذة الرئيسية بعد تسجيل الدخول الناجح"""
        print(f"تم تسجيل دخول المستخدم: {user_data['username']}")
        
        # جلب صلاحيات المستخدم (يمكنك استبدال هذا بمنظومة الصلاحيات الفعلية)
    ...
    def get_user_permissions(self, user_id):
        """جلب صلاحيات المستخدم (تنفيذ مؤقت - يمكنك استبداله بمنظومة الصلاحيات الحقيقية)"""
        # هذه صلاحيات افتراضية للتجربة
        return {
...
class AccountingERP(QMainWindow):
    def __init__(self, user_data, permissions,parent=None):
    ...
    def create_user_toolbar(self):
        ...
        user_btn.setText(f"مرحباً، {self.current_user.get('full_name', self.current_user['username'])} ({self.current_user['username']})")
        #user_btn.setText(f"المستخدم: {self.user_data.get('full_name', self.user_data['username'])} ({self.user_data['username']})")
        user_btn.setIcon(QApplication.style().standardIcon(QStyle.SP_ComputerIcon))
        user_btn.setPopupMode(QToolButton.InstantPopup)
        
        # قائمة المستخدم
        user_menu = QMenu()
        user_menu.addAction(QApplication.style().standardIcon(QStyle.SP_FileIcon), "تغيير كلمة المرور", self.change_password)
        user_menu.addAction(QApplication.style().standardIcon(QStyle.SP_DialogCloseButton), "تسجيل الخروج", self.logout)
    ...
    def setup_ui(self):
        ...
        #user_name = self.user_data.get('full_name') or self.user_data.get('username') or "مستخدم غير معروف"
        user_name = self.current_user.get('full_name') or self.current_user.get('username') or "مستخدم غير معروف"
        #self.user_label = QLabel(f"المستخدم: {user_name}")
        ...
        self.add_menu_section("المحاسبة", [
            ("القيود اليومية", NewJournalEntryManagementWindow, "#673AB7"),
    ...
    def setup_screens(self):
        ...
        # استخدام أيقونة بديلة إذا لم توجد الصورة
        welcome_image.setPixmap(QApplication.style().standardIcon(QStyle.SP_DesktopIcon).pixmap(128, 128))
        
        # نص الترحيب
        welcome_label = QLabel(f"مرحباً بك {self.current_user.get('full_name', self.current_user['username'])} في نظام ERP المحاسبي")
    ...
    def show_screen(self, screen_name, window_class=None):
        ...
        if window_class:
            ...
            # تمرير UserManager لشاشة القيود فقط
            if window_class.__name__ == "NewJournalEntryManagementWindow":
                try:
                    from database.manager.admin.NEWuser_manager import NEWUserManager
                    user_manager = NEWUserManager()
                    screen_widget = NewJournalEntryManagementWindow(user_manager=user_manager)
                    print(f"DEBUG: Journal window created with user manager")
                except Exception as e:
                    print(f"ERROR: Failed to create journal window with user manager: {e}")

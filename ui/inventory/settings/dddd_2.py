    ...
    def load_accounts(self):
        try:
            ...
            if account_type:
                ...
                # محاولة البحث مباشرة في جدول الحسابات (إذا كان هناك حقل account_type)
                try:
                    ...
                    if 'account_type' in columns:
                        cur.execute("""
                            SELECT id, acc_code, account_name_ar, account_type 
                            FROM accounts 
                        ...
                        # إذا لم يوجد حقل account_type، نعرض جميع الحسابات النشطة
                        cur.execute("""
                            SELECT a.id, a.acc_code, a.account_name_ar, t.name_ar as type_name
                            FROM accounts a
                            JOIN account_types t ON a.account_type_id = t.id
                    ...
                    # كحل أخير، جلب جميع الحسابات النشطة
                    cur.execute("SELECT id, acc_code, account_name_ar FROM accounts WHERE is_active = 1 ORDER BY acc_code")
                    accounts = cur.fetchall()
                    self.all_accounts = [(acc[0], acc[1], acc[2], "غير محدد") for acc in accounts]
...
class ItemCategoriesUI(QWidget):
    ...
    def setup_ui(self):
        ...
        # خانات ربط الحسابات
        # ==============================
        accounts_group = QGroupBox("الحسابات المرتبطة")
        accounts_layout = QVBoxLayout(accounts_group)

        self.account_fields = {}
        # استخدام أسماء الأنواع كما هي في قاعدة البيانات
        account_types = ["الأصول", "الخصوم", "حقوق الملكية", "الإيرادات", "المصروفات"]
        display_names = ["أصول", "التزامات", "حقوق ملكية", "إيرادات", "مصروفات"]
        
        for i, t in enumerate(account_types):
            row = QHBoxLayout()
            lbl = QLabel(display_names[i] + ":")
            edit = QLineEdit()
            edit.setReadOnly(True)
            btn = QPushButton("...")
            btn.setFixedWidth(40)
            btn.clicked.connect(lambda _, tp=t, e=edit: self.open_account_tree(tp, e))
            
            # زر إزالة الربط
            remove_btn = QPushButton("×")
            remove_btn.setFixedWidth(30)
            remove_btn.setStyleSheet("color: red; font-weight: bold;")
            remove_btn.clicked.connect(lambda _, e=edit: e.clear())
            
            row.addWidget(lbl)
            row.addWidget(edit)
            row.addWidget(btn)
            row.addWidget(remove_btn)
            accounts_layout.addLayout(row)
            self.account_fields[t] = edit

        form_layout.addWidget(accounts_group)
        ...
        self.table.setHorizontalHeaderLabels(["الكود", "الاسم", "الفئة الأب", "الوصف", "معرف", "الحسابات المرتبطة"])
    ...
    def open_account_tree(self, account_type, field):
        ...
        if dlg.exec_() == QDialog.Accepted and dlg.selected_account:
            ...
            # حفظ الربط
            self.account_mappings[account_type] = acc['id']
    ...
    def load_categories(self):
        ...
        try:
            ...
            for row in rows:
                ...
                self.table.setItem(row_idx, 5, QTableWidgetItem("انقر للتعديل"))  # الحسابات
    ...
    def load_account_links(self):
        ...
        try:
            ...
            # التحقق من وجود جدول لحسابات الفئات
            cur.execute("""
                SELECT name FROM sqlite_master 
                WHERE type='table' AND name='category_accounts'
            """)
            
            if cur.fetchone():
                # إذا كان الجدول موجوداً، جلب البيانات
                cur.execute("""
                ...
                for account_type, account_id in accounts:
                    if account_type in self.account_fields:
                        # جلب اسم الحساب من قاعدة البيانات المالية
                        try:

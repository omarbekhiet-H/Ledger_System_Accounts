...
class InventoryInvoiceUI(QMainWindow):
    ...
    def init_view_tab(self):
        """تبويب عرض الفواتير - مؤقت"""
        layout = QVBoxLayout()
        label = QLabel("سيتم عرض الفواتير هنا لاحقاً")
    ...
    def init_create_tab(self):
        layout = QHBoxLayout()  # الآن التخطيط أفقي (قائمة + النموذج)
    ...
    # --- باقي الدوال مثل load_addition_details, generate_invoice_number, load_addition_items, calculate_totals, save_invoice ... تبقى كما هي في النسخة السابقة ---
    ...
    def view_invoice(self, invoice_id):
        """عرض فاتورة"""
        try:
            invoice, items = self.inventory_invoice.get_invoice_details(invoice_id)
            if invoice:
                # أنشئ نافذة أو dialog لعرض التفاصيل
                msg = f"عرض الفاتورة رقم {invoice[1]}\nالمورد: {invoice[2]}\nالإجمالي: {invoice[6]}"
        ...
        try:
            ...
            html += f"""
                ...
                <div class='invoice-totals'>
                    <h3>الإجمالي النهائي: {invoice_info[5]:.2f} ريال</h3>
                    <p><strong>ملاحظات:</strong> {invoice_info[6] or 'لا توجد ملاحظات'}</p>
                </div>
            ...
            printer.setOutputFileName(f"invoice_{invoice_info[0]}.pdf")
        
            doc = QTextDocument()
            doc.setHtml(html)
            doc.print_(printer)
        
            QMessageBox.information(self, "طباعة", f"تم إنشاء ملف PDF: invoice_{invoice_info[0]}.pdf")
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            invoice_data = {
                ...
                'الإجمالي النهائي': [invoice_info[5]],
                'ملاحظات': [invoice_info[6] or '']
            }
            ...
            # التصدير إلى إكسل
            filename = f"invoice_{invoice_info[0]}.xlsx"
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                invoice_df.to_excel(writer, sheet_name='معلومات الفاتورة', index=False)
    ...
    def view_invoice(self, invoice_id):
        ...
        try:
            ...
            QMessageBox.information(self, "عرض", f"عرض الفاتورة رقم {invoice_id}")
        except Exception as e:
    ...
    def edit_invoice(self, invoice_id):
        ...
        try:
            # هنا يمكنك تنفيذ منطق التعديل
            QMessageBox.information(self, "تعديل", f"تعديل الفاتورة رقم {invoice_id}")
        except Exception as e:
            QMessageBox.critical(self, "خطأ", f"فشل في التعديل: {e}")
    ...
    def clear_form(self):
        ...
        self.total_discount_label.setText("0.00")

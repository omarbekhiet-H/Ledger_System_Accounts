...
class FixedAssetCardUI(QWidget):
    ...
    def setup_financial_info_tab(self):
        ...
        # Row 0 - طريقة الإهلاك
        layout.addWidget(QLabel("طريقة الإهلاك:"), 0, 0)
        self.depreciation_method_combo = QComboBox()
        self.load_depreciation_methods()
        layout.addWidget(self.depreciation_method_combo, 0, 1)

        # Row 1 - سعر الوحدة والتكلفة
        ...
        layout.addWidget(self.unit_price_spin, 1, 1)

        layout.addWidget(QLabel("التكلفة الإجمالية:"), 1, 2)
        self.acquisition_cost_edit = QLineEdit()
        self.acquisition_cost_edit.setReadOnly(True)
        layout.addWidget(self.acquisition_cost_edit, 1, 3)

        # Row 2 - القيمة التخريدية
        layout.addWidget(QLabel("القيمة التخريدية:"), 2, 0)
        ...
        self.salvage_value_spin.valueChanged.connect(self.calculate_depreciation)
        layout.addWidget(self.salvage_value_spin, 2, 1)

        # Row 3 - معدل الإهلاك والعمر
        layout.addWidget(QLabel("معدل الإهلاك %:"), 3, 0)
        ...
        self.depreciation_rate_spin.valueChanged.connect(self.update_useful_life_from_rate)
        layout.addWidget(self.depreciation_rate_spin, 3, 1)

        layout.addWidget(QLabel("العمر الإنتاجي (سنوات):"), 3, 2)
        self.useful_life_spin = QSpinBox()
        self.useful_life_spin.setMinimum(1)
        self.useful_life_spin.setMaximum(100)
        self.useful_life_spin.valueChanged.connect(self.update_rate_from_useful_life)
        layout.addWidget(self.useful_life_spin, 3, 3)

        # Row 4 - مصاريف إضافية
        layout.addWidget(QLabel("مصاريف إضافية:"), 4, 0, 1, 4)
        self.additional_costs_table = QTableWidget(0, 2)
        self.additional_costs_table.setHorizontalHeaderLabels(["الوصف", "المبلغ"])
        self.additional_costs_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        layout.addWidget(self.additional_costs_table, 5, 0, 1, 4)

        # زر لإضافة صف جديد
        self.add_cost_btn = QPushButton("➕ إضافة مصروف")
        self.add_cost_btn.clicked.connect(self.add_additional_cost)
        layout.addWidget(self.add_cost_btn, 6, 0, 1, 4)

        # Row 7 - الإهلاك السنوي
        layout.addWidget(QLabel("الإهلاك السنوي:"), 7, 0)
        self.annual_depreciation_edit = QLineEdit()
        self.annual_depreciation_edit.setReadOnly(True)
        layout.addWidget(self.annual_depreciation_edit, 7, 1)

        # Add stretch
        layout.setRowStretch(8, 1)
    
    def update_useful_life_from_rate(self):
        """حساب العمر من المعدل"""
        rate = self.depreciation_rate_spin.value()
        if rate > 0:
            years = round(100 / rate)
            self.useful_life_spin.blockSignals(True)
            self.useful_life_spin.setValue(years)
            self.useful_life_spin.blockSignals(False)
        self.calculate_depreciation()

    def update_rate_from_useful_life(self):
        """حساب المعدل من العمر"""
        years = self.useful_life_spin.value()
        if years > 0:
            rate = round(100 / years, 2)
            self.depreciation_rate_spin.blockSignals(True)
            self.depreciation_rate_spin.setValue(rate)
            self.depreciation_rate_spin.blockSignals(False)
        self.calculate_depreciation()

    def add_additional_cost(self):
        """إضافة صف جديد للمصاريف الإضافية"""
        row = self.additional_costs_table.rowCount()
        self.additional_costs_table.insertRow(row)
        self.additional_costs_table.setItem(row, 0, QTableWidgetItem(""))
        self.additional_costs_table.setItem(row, 1, QTableWidgetItem("0"))

    def calculate_depreciation(self):
        """حساب الإهلاك السنوي"""
        try:
            base_cost = self.unit_price_spin.value() * self.quantity_spin.value()
            salvage = self.salvage_value_spin.value()

            # جمع المصاريف الإضافية
            additional_costs = 0
            for row in range(self.additional_costs_table.rowCount()):
                try:
                    val = float(self.additional_costs_table.item(row, 1).text())
                    additional_costs += val
                except:
                    pass
            ...
            years = self.useful_life_spin.value()
            if years > 0:
                annual_dep = (total_cost - salvage) / years
                self.annual_depreciation_edit.setText(f"{annual_dep:,.2f}")

        except Exception as e:
            print(f"Error in calculate_depreciation: {e}")
